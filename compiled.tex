...
Working memory (WM) is indispensable for numerous cognitive functions, but the intricate neural mechanisms involved are not fully understood. The hippocampus and sharp-wave ripple complexes (SWRs) --- brief, synchronous neural events in the hippocampus --- attract attention due to their importance in memory consolidation and retrieval; however, their association with WM tasks remains unclear. Our current study posits that hippocampal multiunit activity patterns, in conjunction with SWRs, show distinctive dynamics during WM tasks. We analyzed an intracranial electroencephalogram dataset obtained from the medial temporal lobe (MTL) of nine epilepsy patients performing an eight-second Sternberg task. Using Gaussian-process factor analysis, we derived low-dimensional neural representations, or 'trajectories', within the MTL regions during the WM task. Our results demonstrate that, compared to the entorhinal cortex and amygdala, the hippocampus exhibits the most significant variation in neural trajectory. Moreover, the difference in trajectories between encoding and retrieval phases is contingent upon memory load. Notably, hippocampal trajectories oscillate during the retrieval phase, indicating task-dependent shifts between encoding and retrieval states, both during baseline and SWR events. This oscillation transitions from encoding to retrieval states concurrent with SWRs. Our findings underscore the essential role of the hippocampus in WM tasks and propose a new hypothesis: the hippocampus transitions its functional state from encoding to retrieval during SWRs.
