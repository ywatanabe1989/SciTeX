...
Working memory (WM) forms a crucial foundation for a multitude of cognitive functions, yet the complex neural mechanisms underlying its operation are not fully understood. More specifically, while the hippocampus and sharp-wave ripple complexes (SWRs) --- rapid, coordinated neuronal activities within the hippocampus --- are known to play roles in memory consolidation and retrieval, their contribution to WM tasks remains somewhat ambiguous. Our current research suggests that the multiunit activity patterns in the hippocampus operate in synergy with SWRs, exhibiting distinguishable dynamics during WM tasks. We conducted a comprehensive analysis of a dataset derived from intracranial electroencephalogram recordings of the medial temporal lobe (MTL) from nine epilepsy patients performing an eight-second Sternberg task. We utilized Gaussian-process factor analysis to identify low-dimensional neuronal representations, or 'trajectories,' within the MTL regions during the WM task. Our data showed that the neuronal trajectory varied most significantly in the hippocampus compared to the entorhinal cortex and amygdala. Furthermore, variations in the trajectories between the encoding and retrieval phases were dependent on memory load. Notably, hippocampal trajectories oscillated during the retrieval phase, showcasing task-dependent transitions between encoding and retrieval states, including baseline and SWR periods. These oscillations transitioned from encoding to retrieval states aligned with SWRs. Thus, our findings emphasize the vital role of the hippocampus in carrying out WM tasks and propose a compelling hypothesis for future investigation: the functional state of the hippocampus transitions from encoding to retrieval during SWRs.
