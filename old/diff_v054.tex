...
%DIF DEL ./old/compiled_v053.tex   Sun Jan 14 09:00:32 2024
%DIF ADD ./compiled.tex            Sun Jan 14 09:09:32 2024
...
\\
\indent
Considering these factors, this study investigates the hypothesis that hippocampal neurons \DIFdelbegin \DIFdel{show }\DIFdelend \DIFaddbegin \DIFadd{exhibit }\DIFaddend unique 'neural trajectories' in low-dimensional \DIFdelbegin \DIFdel{spaces}\DIFdelend \DIFaddbegin \DIFadd{space}\DIFaddend , particularly during SWR periods, in response to WM tasks \DIFaddbegin \DIFadd{in humans}\DIFaddend . To test this hypothesis, we employed a dataset of patients performing an eight-second Sternberg task (1 s for fixation, 2 s for encoding, 3 s for maintenance, and 2 s for retrieval) with high temporal resolution. Intracranial electroencephalography (iEEG) signals within the medial temporal lobe (MTL) were recorded for these patients \cite{boran_dataset_2020}. To \DIFdelbegin \DIFdel{examine }\DIFdelend \DIFaddbegin \DIFadd{investigate }\DIFaddend low-dimensional neural trajectories, we utilized Gaussian-process factor analysis (GPFA), an established method for analyzing neural population dynamics \cite{yu_gaussian-process_2009}.
