...
The working memory (WM), a cornerstone of many cognitive functions, holds complex neural mechanisms that are not yet entirely understood. Specifically, the involvement of the hippocampus and sharp-wave ripple complexes (SWRs) -- rapid, coincident neural events within the hippocampus -- in WM tasks remains somewhat uncertain, despite their established roles in memory consolidation and retrieval. In our current research, we propose that multiunit activity patterns within the hippocampus interact synergistically with SWRs, thereby displaying unique dynamics during WM tasks. This study involved an extensive analysis of a dataset obtained from intracranial electroencephalogram recordings from the medial temporal lobe (MTL) of nine patients with epilepsy, while performing an eight-second Sternberg task. Gaussian-process factor analysis was employed to identify low-dimensional neural representations, or 'trajectories,' within the MTL during the WM task. We found that the hippocampus exhibited the most significant variations in neural trajectories compared to the entorhinal cortex and the amygdala. Interestingly, the divergence in trajectories between encoding and retrieval phases was memory load-dependent. More specifically, hippocampal trajectories oscillated during the retrieval phase, revealing task-dependent transitions between encoding and retrieval states, encapsulating baseline and SWR episodes. These oscillations moved from encoding to retrieval states correlating with the SWRs. Therefore, these findings emphasize the essential role of the hippocampus in performing WM tasks and suggest a compelling premise for further investigation: the functional state of the hippocampus transitions from encoding to retrieval during SWRs.
