...
Working memory (WM) is integral to numerous cognitive functions, but the complex neural mechanisms essential for its operation are not entirely understood. Specifically, the roles of the hippocampus and sharp-wave ripple complexes (SWRs) -- rapid, synchronised neural events within the hippocampus -- are recognized to support memory consolidation and retrieval. However, their contributions to WM tasks remain somewhat unclear. We suggest that the coordinated activity patterns in the hippocampus collaborate with SWRs, displaying unique dynamics during WM tasks. Our investigation involved a comprehensive analysis of a dataset acquired from intracranial electroencephalogram recordings from the medial temporal lobe (MTL) of nine epileptic patients during an eight-second Sternberg task. We use Gaussian-process factor analysis to identify low-dimensional neural representations, or 'trajectories,' within the MTL regions during the WM task. We discovered that the neural trajectory showed the most significant variations in the hippocampus compared to the entorhinal cortex and amygdala. Furthermore, we noticed that the discrepancy in trajectories between encoding and retrieval phases was dependent on memory load. Interestingly, hippocampal trajectories oscillated during the retrieval phase, revealing task-dependent shifts between encoding and retrieval states, including baseline and SWR phases. These oscillations transitioned from encoding to retrieval states congruent with the occurrence of SWRs. These results highlight the substantial role of the hippocampus during WM tasks execution and propose a persuasive hypothesis for further examination: the hippocampus undergoes a functional transition from encoding to retrieval during SWRs.
