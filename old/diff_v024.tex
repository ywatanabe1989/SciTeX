...
%DIF DEL ./old/compiled_v023.tex   Sat Jan 13 20:33:31 2024
%DIF ADD ./compiled.tex            Sat Jan 13 20:36:09 2024
...
\DIFdelendFL \DIFaddbeginFL \thead{\# of total sessions}\DIFaddendFL } &\textbf{\thead{AHL}} &\textbf{\thead{AHR}} &\textbf{\thead{PHL}} &\textbf{\thead{PHR}} &\textbf{\thead{ECL}} &\textbf{\thead{ECR}} &\textbf{\thead{AL}} &\textbf{\thead{AR}} &\textbf{\thead{SOZ}} &\\
\midrule
...
\\
The silhouette scores (mean \textpm SD across sessions per subject) for UMAP clustering of \DIFdelbeginFL \textbf{\DIFdelFL{$SWR^+$}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mathrm{$SWR^+$} }\DIFaddendFL candidates and \DIFdelbeginFL \textbf{\DIFdelFL{$SWR^-$}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mathrm{$SWR^-$} }\DIFaddendFL candidates are based on their respective multiunit spike patterns (Figure~\ref{fig:04}A). The mean scores were 0.205 and the standard deviation was 0.285, calculated for the interquartile range (IQR; Figure~\ref{fig:04}B).}
% width=1\textwidth
\label{tab:02}
\end{table*}
\restoregeometry
