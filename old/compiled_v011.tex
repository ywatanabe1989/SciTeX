...
Working memory (WM) is a vital component in numerous cognitive functions, yet the complex neural mechanisms that sustain its operation are not fully understood. In particular, while the hippocampus and sharp-wave ripple complexes (SWRs) --- brief, coordinated neural events within the hippocampus --- are known for their roles in memory consolidation and retrieval, their engagement in WM tasks remains unclear. Our current research posits that the multiunit activity patterns in the hippocampus collaborate with SWRs, showing unique dynamism during WM tasks. Our study involved the analysis of a dataset obtained from intracranial electroencephalogram recordings procured from the medial temporal lobe (MTL) of nine individuals with epilepsy performing an eight-second Sternberg task. We utilized Gaussian-process factor analysis to discern low-dimensional neural representations, or 'trajectories,' within the MTL regions during the WM task. Our results revealed that the neural trajectories showed the most pronounced variations in the hippocampus compared to the entorhinal cortex and amygdala. Furthermore, differences in the trajectories between encoding and retrieval phases were dependent on memory load. Crucially, hippocampal trajectories oscillated during the retrieval phase, illustrating task-dependent transitions between encoding and retrieval stages, which include baseline and SWR episodes. These oscillations transitioned from encoding to retrieval states in accordance with SWRs. Therefore, these outcomes demonstrate the integral role of the hippocampus in executing WM tasks and put forth a compelling hypothesis for further investigation: the operational state of the hippocampus shifts from encoding to retrieval during SWRs.
